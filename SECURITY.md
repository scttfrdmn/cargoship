...
The release trust anchor is this cosign provenance chain — **not** the git tag's
own signature. Depending on how a given tag was created, GitHub may show its
SSH/GPG signature as unverified; that is not a release-integrity signal either
way, so verify the artifacts with the cosign flow above rather than relying on
the tag's signature badge.
