...
## Rotating GPG keys made before v0.23.0

Private keys generated by `cargoship create keys` before v0.23.0 are
**unencrypted**, and were written to `$TMPDIR` by default. If you hold such a key,
see [Rotating a key generated before v0.23.0](/guides/features/encryption#rotating-pre-v0230-keys).
Because archives are encrypted to the public key, rotating protects future
uploads but does not protect data already uploaded to the old key.
