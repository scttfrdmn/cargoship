...
func BenchmarkPipeline_SmallFiles_10K(b *testing.B) {
	...
	if useRealS3 {
		...
		config.EnableMultiPrefix = true // Phase 3.1: Use multi-prefix parallel uploads
		config.ShardCount = 8           // 8 shards for optimal S3 partition parallelism
		config.WorkersPerPrefix = 2     // 2 workers per shard = 16 total workers
