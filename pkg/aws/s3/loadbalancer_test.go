...
func TestLoadBalanceOptimization(t *testing.T) {
	...
	// Verify that weights are properly normalized and adjusted
	// After optimization: prefix-1 (1.0->1.1), prefix-2 (1.5 unchanged), prefix-3 (0.8->0.88)
	// Then normalized by factor of 3/3.48 ≈ 0.862

	// Check that good performers maintain relative advantage
	initialRatio32 := 0.8 / 1.5 // prefix-3 vs prefix-2 initially
	finalRatio32 := solution.Weights["prefix-3"] / solution.Weights["prefix-2"]
	assert.Greater(t, finalRatio32, initialRatio32) // Relative position should improve

	// Verify basic constraints
	assert.Greater(t, solution.Weights["prefix-3"], 0.0)
	assert.Greater(t, solution.Weights["prefix-2"], 0.0)
	assert.Greater(t, solution.Weights["prefix-1"], 0.0)
}
