...
func (mm *MemoryMonitor) GetPressureLevel(usage float64) MemoryPressureLevel {
	...
	// Use ordered comparisons instead of unreliable map iteration
	if usage <= mm.pressureThresholds[MemoryPressureNone] {
		return MemoryPressureNone
	}
	if usage <= mm.pressureThresholds[MemoryPressureLow] {
		return MemoryPressureLow
	}
	if usage <= mm.pressureThresholds[MemoryPressureModerate] {
		return MemoryPressureModerate
	}
	if usage <= mm.pressureThresholds[MemoryPressureHigh] {
