...
// createSuitcaseLegacyCmd represents the createSuitcase command
var createSuitcaseLegacyCmd = &cobra.Command{
	Use:   "suitcaselegacy DESTINATION_DIR",
...
func init() {
	createCmd.AddCommand(createSuitcaseLegacyCmd)

	// Here you will define your flags and configuration settings.

	// Cobra supports Persistent Flags which will work for this command
	// and all subcommands, e.g.:
	createSuitcaseLegacyCmd.PersistentFlags().StringP("inventory", "i", "", "Inventory file for the suitcase")
	err := createSuitcaseLegacyCmd.MarkPersistentFlagRequired("inventory")
	checkErr(err, "")
	createSuitcaseLegacyCmd.PersistentFlags().Bool("hash-outer", false, "Create SHA256 hashes for the suitcases")
	createSuitcaseLegacyCmd.PersistentFlags().Bool("exclude-systems-pubkeys", false, "By default, we will include the systems teams pubkeys, unless this option is specified")
	createSuitcaseLegacyCmd.PersistentFlags().Int("concurrency", 10, "Number of concurrent files to create")
	createSuitcaseLegacyCmd.PersistentFlags().String("progress-bar", "none", "Progress bar to use. Valid options are: 'mpb' or 'none'")

	createSuitcaseLegacyCmd.Flags().StringArrayP("public-key", "p", []string{}, "Public keys to use for encryption")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// createSuitcaseLegacyCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
