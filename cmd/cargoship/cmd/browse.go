...
func NewBrowseCmd() *cobra.Command {
	var (
		...
		restoreDays    int32
		datasetID      string // #603: version-aware browse
		version        int
		asOfStr        string
	)
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// #603: version-aware browse. With --dataset-id the S3_URL is a
			// bucket/prefix; resolve the requested version of the dataset to a
			// concrete upload, then browse its effective view (mirrors restore).
			...
			case datasetID != "":
				...
				if asOfStr != "" {
					...
					asOf = d.Add(24*time.Hour - time.Nanosecond)
				}
				...
				fmt.Printf("🎯 Dataset %s: browsing v%d (upload %s)\n", datasetID, sel.VersionOrdinal, uploadID)
			default:
				if version > 0 || asOfStr != "" {
	...
	cmd.Flags().StringVar(&datasetID, "dataset-id", "", "Browse a version of this dataset (S3_URL is then the bucket/prefix); see 'cargoship dataset list'")
	cmd.Flags().IntVar(&version, "version", 0, "With --dataset-id: the version ordinal to browse (default: latest)")
	cmd.Flags().StringVar(&asOfStr, "as-of", "", "With --dataset-id: browse the newest version at or before this date (YYYY-MM-DD)")
